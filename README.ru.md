<!-- translation: locale=ru; source=README.md; source-sha256=f1223d703e387bd7fbe49b9880d3ada5e653e0a4672b432576ab2c8892421b0b -->
<div align="center">
...
| Параметр | Значение |
| --- | --- |
| Версия проекта | `2.8.0`, пока неопубликованный кандидат Release; факт публикации определяется неизменяемыми Git-тегами и GitHub Releases |
...
| Инженерные снимки M7 | Датированы 2026-07-19: Ubuntu 24.04 arm64/systemd и контейнер Alpine 3.22 arm64/OpenRC; это исторические инженерные результаты |
| Целевые ресурсы | Весь хост `512 MiB / 1 vCPU / 2 GB disk`, максимум сервиса `448 MiB`, без swap |
| Проверки кандидата schema v2 | GitHub CI; сборка образов amd64/arm64 с provenance/build attestation; привязанный к кандидату реальный low-memory smoke на amd64/x86_64 с нормативным шаблоном Compose; Panel 2.8.1 в состоянии online с реальным трафиком; контейнер healthy, OOM/restart равны нулю, наблюдения memory/PID допустимы |
| Отложенная проверка | `arm64-production-runtime`, `native-systemd-install`, `native-openrc-install`, `50000-user-load`, `24h-soak`, `fault-and-rollback-injection` |

Evidence schema v2 представляет собой заверенную оператором запись, привязанную к кандидату. Gate проверяет ее структуру и связь с commit кандидата, образом и нормативным файлом Compose, но не может независимо подтвердить наблюдения оператора и не является доказательством, которое невозможно подделать. Отложенные проверки явно не входят в gate кандидата `2.8.0`, и Release не должен создавать впечатление, что они выполнены. Датированные результаты M6/M7 остаются полезными инженерными baseline, а не приемкой кандидата или опубликованным SLA; см. [roadmap, English](docs/development/roadmap.md).
...
Приведённые выше candidate tags служат только для обнаружения образа. До начала
`docker-production-smoke-v1` разрешите tag в manifest digest и замените
`image:` на `ghcr.io/luxiaba/remnanode-lite@sha256:<manifest-digest>`.
Compose-файл со ссылкой на tag не является допустимым smoke evidence.
...
| --- | --- | --- |
| `sha-<commit>` | Автоматический кандидат для commit в `main` | Найти кандидат, затем разрешить и зафиксировать manifest digest |
| `candidate-sha-<commit>` | Независимый кандидат из ручного запуска workflow на `main` | Найти ручную пересборку, затем разрешить и зафиксировать manifest digest |
