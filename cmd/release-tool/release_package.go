...
func assembleReleasePackage(options assembleOptions) error {
	...
	checksums, err := buildChecksumFile(temporary, releasePayloadNames(options.version))
	if err != nil {
		return err
	}
...
func verifyReleasePackage(options verifyPackageOptions) error {
	...
	wantChecksums, err := buildChecksumFile(options.directory, releasePayloadNames(options.version))
	if err != nil {
		return err
	}
...
func runVerifyPackage(args []string, stdout, stderr io.Writer) error {
	...
	fmt.Fprintf(stdout, "verified complete release package: %s\n", options.directory)
	return nil
}
