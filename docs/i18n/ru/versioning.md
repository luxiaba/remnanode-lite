<!-- translation: locale=ru; source=docs/versioning.md; source-sha256=16e3127d053d9a540a42607f0139e89e4b6cd163bbc1916a6e124930dcae7d6f -->
...
Это preview Remnanode Lite, который улучшает работу администратора с Native и продолжает реализовывать проверенный контракт официального Node `2.8.0`. Native Linux distribution появляется только после публикации соответствующего GitHub Release. Суффикс `rnl.N` принадлежит этому проекту, а не официальному выпуску. Изменение только `Version` не расширяет заявленный контракт.
