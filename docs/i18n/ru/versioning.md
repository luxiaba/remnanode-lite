<!-- translation: locale=ru; source=docs/versioning.md; source-sha256=955b65a17ad16617db4308fd47d0f8d4295a620e095775e924f61d7e73df6473 -->
...
Это preview Remnanode Lite, который улучшает Native delivery и продолжает реализовывать проверенный контракт официального Node `2.8.0`. Native Linux distribution появляется только после публикации соответствующего GitHub Release. Суффикс `rnl.N` принадлежит этому проекту, а не официальному выпуску. Изменение только `Version` не расширяет заявленный контракт.
...
| --- | --- | --- | --- |
| `2.8.0` | `2.8.0` | Stable | Опубликованная базовая версия контракта и текущая линия `latest` |
| `2.8.0-rnl.1` | `2.8.0` | Preview | Улучшения Native administration и поддержки Alpine; точные assets появляются после публикации |
