<!-- translation: locale=ru; source=docs/operations.md; source-sha256=797125eeff49198bb88d97ceb0b20197df49bd3a7bb814827d15d017a1f3572b -->
...
`overview` — первая проверка для оператора. Команда объединяет состояние
жизненного цикла и службы, версию и generation, настроенный endpoint без Secret,
известные проблемы и краткий список команд для текущего состояния. Она читает
только локальный status и безопасную администраторскую конфигурацию, не
обращается к Panel и не создаёт proxy-трафик. JSON-формы нет, а формат для
человека не является интерфейсом для разбора.
...
После успешных `install`, `activate`, `upgrade`, `rollback` и `repair` выводятся
зависящие от состояния команды `Next`. При сбое изменения жизненного цикла в
stderr могут добавляться безопасные локальные команды `status`, `doctor` или
просмотра журнала Node. Они никогда не запускаются автоматически, не выводятся
при отмене и скрываются режимом quiet.
...
`overview`/`status`/`doctor`. Он не скрывает help, version, `config show`/`get`, журналы,
...
подключён к TTY: overview, status и doctor проверяют stdout, а прогресс — stderr.
...
Обычно `0` означает успех, `1` — ошибку выполнения или нездоровое состояние, `2` — ошибку в аргументах. `status` и `overview` считают `absent` допустимым состоянием и возвращают `0`; если безопасную сводку конфигурации прочитать нельзя, `overview` возвращает `1`. Скрипту, которому нужна установленная служба, следует использовать `status --json` и проверить `installed` или `deployment`. После запуска `journalctl` или `tail` команда `logs` возвращает его код завершения, в том числе `128 + signal` при завершении сигналом.
...
Текущая стабильная цель — `3.4.1`. Compose-шаблон этого Release выбирает соответствующий
образ и содержит закреплённый asset GeoCheck. Сохраните `.env`, явно проверьте новые SNI- и
nftables-параметры, запишите текущий digest, а после пересоздания проверьте связь с Panel,
GeoCheck, rw-core и репрезентативный proxy-трафик.
...
После изменения Docker `.env` или Compose mapping проверьте модель и заново создайте контейнер. В Native единственным источником настроек службы служит `/etc/remnanode-lite/node.env`; `rnlctl config` читает и изменяет его напрямую и показывает только девять администраторских полей без Secret. Secret и внутренние управляемые поля не выводятся.
