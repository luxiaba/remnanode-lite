<!-- translation: locale=ru; source=docs/deployment-docker.md; source-sha256=cc59c6b2b1526f2f085f4e8a678ec09420077b55ce4d8b057d6f48b2c76dcce0 -->
...
Это строгие пределы cgroup контейнера, которые должны сохраняться и на более крупном Docker-хосте. Профиль `docker-production-smoke-v2` можно выполнить на таком хосте, но он проверяет предел памяти `448 MiB`, общий предел памяти и swap `448 MiB`, `1 CPU` и `256 PIDs`. Равные пределы памяти и памяти вместе со swap не оставляют контейнеру дополнительного swap. Отдельная проверка целой машины `512 MiB RAM / 1 vCPU / 2 GB disk` отложена; контейнерный smoke нельзя представлять как доказательство этой цели. Пределы также не являются SLA для любого профиля трафика или набора плагинов. Измерения и границы приведены в [каноническом английском документе о бюджете ресурсов](../../development/resource-budget.md).
...
Выберите полный тег `sha-<40-character-commit>` или `candidate-sha-<40-character-commit>` в [пакете GHCR](https://github.com/luxiaba/remnanode-lite/pkgs/container/remnanode-lite) и замените значения-заполнители порта и Secret до запуска. Для формальной приёмки определите digest манифеста и задайте в `.env` `REMNANODE_IMAGE=ghcr.io/luxiaba/remnanode-lite@sha256:<manifest-digest>`. Кандидаты предназначены для тестирования и не должны выдаваться за выпущенные версии.
...
## Миграция с официального или устаревшего контейнера

`NODE_PORT` и полный `SECRET_KEY` из официального образа `remnawave/node` остаются действительными. Они относятся к внешнему контракту Panel-to-Node, а не к внутренней структуре Node.js и s6 официального образа. Та же процедура применяется к старому шаблону Remnanode Lite, где сервис и контейнер назывались `remnanode`. Никогда не запускайте `remnanode` и `remnanode-lite` одновременно: при host networking они будут конкурировать за порт Node API и входящие proxy-порты.
...
4. Выполните `down --remove-orphans` для нового проекта Compose. Если контейнер `remnanode` всё ещё существует, потому что принадлежал другому проекту Compose, проверьте его, убедитесь, что это старый Node, и явно удалите до запуска замены.
...
Если проверка вывела контейнер, вручную убедитесь по имени и образу, что это старый Node. Только после этого выполните отдельную команду удаления:

```bash
docker rm -f remnanode
```

После удаления `remnanode` запустите и проверьте замену:
...
Профиль `docker-production-smoke-v2` требует не менее 600 секунд проверки на реальном нативном хосте `linux/amd64` / `x86_64` с эталонным Compose и ссылкой по digest манифеста. Хост может иметь больше CPU, памяти, диска или host swap, чем целевая целая машина; запись вместо этого проверяет точные пределы cgroup контейнера, его состояние, память и PID, связь с Panel, запуск rw-core, реальный прокси-трафик, OOM и число перезапусков. Проверка целой машины `512 MiB RAM / 1 vCPU / 2 GB disk / zero swap`, `arm64-production-runtime`, нативная установка systemd/OpenRC, нагрузка в 50 000 пользователей, 24-часовой прогон и испытания отказа и отката остаются последующей работой и не блокируют `v2.8.0`. Полные требования и формат записи приведены в [протоколе приёмки (англ.)](../../development/release-acceptance.md#docker-production-smoke).
