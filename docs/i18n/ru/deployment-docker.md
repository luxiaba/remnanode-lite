<!-- translation: locale=ru; source=docs/deployment-docker.md; source-sha256=ab16405a98512e44f3172aceeb6efa3f511da05d1b97e73cce9a4c72acee171d -->
...
- именованный том `remnanode-state` в `/var/lib/remnanode-lite` для удаляемого производного кэша Core/GeoData от Panel. Корневая файловая система остаётся доступной только для чтения; пересоздание контейнера по-прежнему очищает копии конфигурации времени выполнения и журналы, после чего Panel повторно отправляет конфигурацию Xray.
...
| --- | --- | --- |
| `X.Y.Z-rnl.N` | Точный preview-релиз проекта, прошедший процесс выпуска | Только для контролируемого preview-тестирования и точного отката |
...
| `preview` | Последний продвинутый prerelease `rnl.N` | Осознанное слежение за preview-каналом; не идентификатор отката |
...
Когда Panel выбирает пользовательский Core или GeoData, Node сохраняет производные файлы в `/var/lib/remnanode-lite/panel-runtime/{assets,cores}` на томе `remnanode-state`. Том сохраняется при обычном пересоздании контейнера, обновлении, откате и `docker compose down`, но не содержит полной конфигурации Xray, Secret или журналов. Эти файлы не покрываются `runtime-assets.lock.json`, SBOM или provenance образа; встроенные в образ Core и GeoData остаются резервным вариантом.

Первичные загрузки Node требуют HTTPS. Каждая загрузка ограничена `128 MiB`, общим временем `15s` и прерывается после `5s` без данных; одновременно загружается не более пяти ресурсов GeoData. Пользовательский Core должен также пройти проверку заданного SHA-256 и версии. Позднее rw-core может обновлять GeoData по cron-конфигурации от Panel; на эти записи не распространяются ограничения размера и общего времени первичной загрузки Node. Разрешайте динамические ресурсы только доверенному администратору Panel и контролируйте использование тома и диска хоста.

`docker compose down -v` удаляет тома, управляемые Compose, включая этот кэш; используйте команду только при намеренном удалении кэша. Резервная копия не нужна для корректной работы: синхронизация Panel и встроенный резервный вариант позволяют восстановить состояние времени выполнения.
...
Переносить состояние времени выполнения или конфигурацию Xray из официального контейнера не требуется: новый том `remnanode-state` начинается как пустой производный кэш, а Panel отправит конфигурацию повторно. Для отката сначала остановите `remnanode-lite`, затем восстановите резервный Compose и точный официальный образ. Никогда не оставляйте запущенными оба имени контейнера. Сохраняйте резервную копию, пока новый контейнер не завершит период наблюдения.

## Образы-кандидаты

Для каждого коммита `main` workflow `candidate` (`container.yml`) собирает образы `linux/amd64` и `linux/arm64`, публикует многоархитектурный манифест и записывает происхождение сборки. Затем он публикует `sha-<commit>` и обновляет `edge`, если этот коммит всё ещё является вершиной `main`. Эти проверки показывают, как был собран образ, но не подтверждают его работу.

Перед запуском version release сопровождающий запускает кандидат, зафиксированный по digest, с каноническим Compose-файлом. Нужно убедиться, что контейнер переходит в состояние healthy, подключается к реальной Panel, запускает rw-core и пропускает типичный для этого узла прокси-трафик. Даже на более крупном хосте сохраняются ограничения контейнера: `448 MiB / 1 CPU / без дополнительной подкачки контейнера / 256 PIDs`.
...
Для текущего стабильного Release `3.4.1` используйте приложенный Compose-шаблон: он выбирает
образ `3.4.1`, содержит закреплённый runtime asset GeoCheck и явно передаёт новые SNI- и
nftables-параметры. Сохраните `.env`, скачайте точный образ или digest и после пересоздания
проверьте связь с Panel и реальный трафик.

Проверка derived SNI теперь необязательна и по умолчанию выключена; TLS 1.3, mTLS, JWT и
целостность Secret остаются обязательными. Включайте `SNI_VERIFICATION=true` только если все
Panel отправляют derived server name. На нагруженном хосте проверьте `NFTABLES_LOGGING`;
оставьте `NFTABLES_ACCEPT_REPLY_TRAFFIC=false`, если ingress list не блокирует ответы на
соединения, инициированные хостом, либо conntrack недоступен.

Сделайте резервную копию текущих входных файлов Compose, затем получите и проверьте шаблон Compose из целевого Release. Сохраните `.env` и осознанно повторно примените нужные локальные переопределения, прежде чем менять `REMNANODE_IMAGE` или намеренно inline-значение `image:`. Не используйте новый образ со старым Compose: при обновлении с `3.0.0` до `3.2.2` особенно необходим новый том `remnanode-state`, поскольку корневая файловая система контейнера остаётся read-only. Затем явно скачайте и пересоздайте контейнер:
...
Том `remnanode-state` является постоянным, но содержит только восстанавливаемый производный кэш Core/GeoData от Panel. Его размер зависит от ресурсов и расписания обновлений, выбранных в Panel, и не ограничен пределами tmpfs для журналов. Обычный `docker compose down` сохраняет том, а `docker compose down -v` удаляет его.
...
Эта трассируемость относится к ресурсам, встроенным в образ, а не к удаляемому кэшу Core/GeoData, который позднее загружается с URL, выбранных в Panel.
