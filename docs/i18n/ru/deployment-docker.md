<!-- translation: locale=ru; source=docs/deployment-docker.md; source-sha256=5807f5ad275e0506a0a51fef5020b311a0f30f6a4ef5522e5da93c0117e671ba -->
...
Для текущего стабильного Release `3.3.2` используйте приложенный Compose-шаблон: он выбирает
образ `3.3.2` и содержит закреплённый runtime asset GeoCheck. Сохраните `.env`, скачайте точный
образ или digest и после пересоздания проверьте связь с Panel и реальный трафик.

Сначала обновите Panel до совместимой версии `3.3.x`. Старые версии Panel не отправляют
производный SNI, необходимый TLS-фильтру Node `3.3.2`, поэтому после обновления Node они
