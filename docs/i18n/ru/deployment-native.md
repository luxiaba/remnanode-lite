<!-- translation: locale=ru; source=docs/deployment-native.md; source-sha256=6d3288143ca5ecc8d336d55c83744c32d875ad4d53e89e345e942cb1b48a7e0c -->
...
Запускайте установщик от root. Для онлайн-установки нужны systemd (либо описанная выше среда Alpine/OpenRC), `nft` из nftables, `ss` из iproute2, `useradd` и `groupadd`, если выделенная учётная запись `remnanode-lite` ещё не существует, доверенное хранилище CA, `curl` или GNU Wget с поддержкой `--https-only`, GNU tar и gzip. Порт Node должен быть доступен Panel, а входящие proxy-порты — соответствовать конфигурации Panel.
