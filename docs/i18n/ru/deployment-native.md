...
Пакеты Native выпускаются для Linux на архитектурах `amd64` и `arm64`. Профиль ограничивает службу значениями `448 MiB RAM`, `1 CPU` и `256 tasks` и отключает дополнительный swap при cgroup v2. В стандартной конфигурации Rocky Linux 8 с cgroup v1 использование swap следует политике хоста. Профиль рассчитан так, чтобы оставить запас на хосте `512 MiB / 1 vCPU / 2 GB`.
...
указанные проблемы и команды для текущего состояния. Она читает только
локальный status и безопасную конфигурацию, не обращается к Panel и не создаёт
proxy-трафик. JSON-формы нет, а вывод не является контрактом для разбора.
...
| `recovery-required` | Остался незавершённый journal или состояние жизненного цикла нечитаемо; перед repair изучите указанную проблему |
...
зависящие от состояния подсказки `Next`. При сбое изменения жизненного цикла в
stderr могут добавляться безопасные локальные команды `status`, `doctor` или
просмотра журнала Node. Подсказки только печатаются, никогда не запускаются
автоматически и не добавляются при отмене.

`--quiet` (или `-q`) скрывает прогресс, сообщения об успешных изменениях и их
подсказки, подсказки при сбое, успешный результат `config check` и
человекочитаемый вывод `overview`/`status`/`doctor`. Он имеет приоритет над явно
выбранным режимом прогресса, но не скрывает help, version, `config show`/`get`,
журналы, автодополнение, планы dry-run, JSON и исходную строку ошибки.

Сдержанные цвета используются только тогда, когда соответствующий поток подключён
к TTY: overview, status и doctor проверяют stdout, а прогресс — stderr. `--no-color`,
...
Обычно код `0` означает успех, `1` — ошибку выполнения или нездоровое состояние, а `2` — ошибку в аргументах. `status` и `overview` считают `absent` допустимым состоянием и возвращают `0`; если безопасную сводку конфигурации прочитать нельзя, `overview` возвращает `1`. Автоматизации, которой нужна установленная служба, следует использовать `status --json` и проверить `installed` или `deployment`. После запуска `journalctl` или `tail` команда `logs` возвращает код завершения этого процесса, в том числе `128 + signal` при завершении сигналом.
