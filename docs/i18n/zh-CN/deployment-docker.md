<!-- translation: locale=zh-CN; source=docs/deployment-docker.md; source-sha256=cc59c6b2b1526f2f085f4e8a678ec09420077b55ce4d8b057d6f48b2c76dcce0 -->
...
这些是严格的容器 cgroup 限制，即使 Docker 宿主机更大也必须保持。`docker-production-smoke-v2` 可以在较大宿主机上执行，但会核验容器仍精确限制为 `448 MiB` 内存上限、`448 MiB` 内存与 swap 合计上限、`1 CPU` 和 `256 PIDs`。两个 448 MiB 上限相等，因此容器没有额外 swap 配额。整机 `512 MiB RAM / 1 vCPU / 2 GB disk` 目标需要另行验证，目前属于延后工作；不能把容器 smoke 当作整机目标已经通过的证明。这些限制也不是对任意流量和插件组合的 SLA。资源证据与边界见[资源预算](development/resource-budget.md)。
...
从 [GHCR Package](https://github.com/luxiaba/remnanode-lite/pkgs/container/remnanode-lite) 选择完整的 `sha-<40位commit>` 或 `candidate-sha-<40位commit>` 标签，启动前替换端口和 Secret 占位值。正式验收时，还要把 tag 解析为 manifest digest，并在 `.env` 中设置 `REMNANODE_IMAGE=ghcr.io/luxiaba/remnanode-lite@sha256:<manifest-digest>`。候选镜像只是测试构建，不能当作正式版本发布。
...
`docker-production-smoke-v2` profile 要求在真实原生 `linux/amd64` / `x86_64` 主机上完成至少 600 秒的 smoke test，并使用规范 Compose 和 manifest digest。宿主机的 CPU、内存、磁盘或 host swap 可以大于整机目标；证据改为核验精确容器 cgroup 限制、健康状态、内存和 PID 观测、Panel 连接、rw-core 启动、真实代理流量、OOM 状态和重启次数。整机 `512 MiB RAM / 1 vCPU / 2 GB disk / zero swap` 验证、`arm64-production-runtime`、原生 systemd/OpenRC 安装、5 万用户负载、24 小时持续运行，以及故障与回滚注入属于后续工作，不阻断 `v2.8.0`。完整要求和记录格式见[发布验收协议](development/release-acceptance.md#docker-生产-smoke)。
