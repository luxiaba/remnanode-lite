...
- tag ruleset 必须允许 Releases API 在公开 draft 时创建 `v*` tag，同时禁止普通用户和工作站创建、更新或删除这类 tag。应明确配置 bypass policy，只让受保护的发布自动化走这条路径；公开后由 Release immutability 保护 tag。
...
9. 以正确的稳定版或预发布状态公开 draft；`v<version>` tag 在这一步创建。稳定版成为 GitHub Latest 之前，精确镜像已经可用。
