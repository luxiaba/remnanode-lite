<!-- translation: locale=zh-CN; source=docs/release.md; source-sha256=7e08e6c881bf6d3c737a990ab15564e0e4d33286fc4c2469fedad9b77758aed1 -->
...
当前稳定发布使用 `VERSION=2.8.0` 与 `TAG=v2.8.0`。推送 tag 会启动 Release
workflow；在初始源码身份检查通过前保持 `main` 不变。仓库规则应禁止更新和删除 `v*`，
workflow 也会在建立 draft、发布 Release 和推进通道前重新解析远端 annotated tag。如果
之后发现源码缺陷，应在 `main` 修复并使用新版本，不要移动已发布 tag。
