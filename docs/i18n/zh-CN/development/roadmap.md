<!-- translation: locale=zh-CN; source=docs/development/roadmap.md; source-sha256=efcca93202c38638f2d9d5b9796d368c1f6b1ca86c5cdcc0614cdac67b8e6d58 -->
# Remnanode Lite 路线图
...
- `/node` 路由严格遵循官方 Node 3.0.0 的 HTTP 方法、请求、响应和错误语义。
...
| M10 官方 Node 3.0.0 对齐 | 实现已完成；等待候选验收 |
...
`rnlctl` 检查与状态相关操作提示。`3.0.0` 源码线已经对齐当前官方契约，发布前仍需完成
不可变候选验收。
运行观测不写入源码仓库，Release notes 由 GitHub 自动生成。

## 当前重点

- **当前**：使用兼容 Panel 验证不可变的 `3.0.0` 候选，确认内置 rw-core 启动和真实
  代理流量；使用支持 pre-start 的版本（`3.1.0` 或更新版本）验证陈旧 socket 清理行为。
  服务器细节与运行观测不写入仓库。
- **发布范围**：未来两个发布周期不新增发布通道、产物类型、发布状态或证明机制；可靠性、
  安全修复和删除重复检查仍在范围内。
- **下一步**：完成 `3.0.0` 发布决策后，继续推进明确列出的 Native 主机验证候选。
