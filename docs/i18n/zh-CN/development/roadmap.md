<!-- translation: locale=zh-CN; source=docs/development/roadmap.md; source-sha256=a0c3304a8f93ca4cfb17632865ecff141f6527c7add973d9e22d7d335ed1bfb4 -->
# Remnanode Lite 路线图
...
- Native 主机支持必须明确并以证据为依据。Rocky Linux 9/systemd 仍是主要目标；
  持续维护的主机配置、Alpine 前置条件和正在验证的发行版统一列在
  [Native 主机矩阵](../deployment-native.md#native-主机矩阵)中。
...
- `/node` 路由严格遵循官方 Node 3.4.1 的 HTTP 方法、请求、响应和错误语义。
...
| M10 官方 Node 3.0.0 对齐 | 已完成 |
| M11 官方 Node 3.2.2 对齐 | 已完成 |
| M12 官方 Node 3.3.2 对齐 | 已完成 |
| M13 官方 Node 3.4.1 对齐 | 实现已完成；等待候选验收 |
...
不改变官方 Node 契约的前提下修正 Alpine 验证说明。已发布的 `2.8.0-rnl.4` 预发布版改进了
`rnlctl` 检查与状态相关操作提示。已发布的稳定版 `3.0.0` 增加了 pre-start socket 清理和
Zod 4 契约边界。`3.2.2` Release 加入可选启动 metadata、integrations 与插件 schema
`0.6.3`。已发布的 `3.3.2` 增加派生 SNI、Secret 完整性、GeoCheck `0.3.0`、Torrent
规则位置与插件 schema `0.7.3`。`3.4.1` 源码线退役两条 Handler 查询路由，使 SNI
校验成为可选项，在凭据替换时清理连接，并加入 nftables 运行选项，同时复核插件
schema `0.8.2`。
运行观测不写入源码仓库，Release notes 由 GitHub 自动生成。

## 当前重点

- **当前**：使用兼容 Panel 验证不可变的 `3.4.1` 候选，确认两种 SNI 模式、凭据
  替换、nftables 选项默认值、GeoCheck、内置 rw-core 启动和真实代理流量。服务器
  细节与运行观测不写入仓库。
- **发布范围**：未来两个发布周期不新增发布通道、产物类型、发布状态或证明机制；可靠性、
  安全修复和删除重复检查仍在范围内。
- **下一步**：完成 `3.4.1` 发布决策后，继续推进明确列出的 Native 主机验证候选。
...
- Native journal 无法自动恢复“主机断电且留下异常 OpenRC cgroup 进程”的状态。
  ...
  重新创建容器。
- Alpine 当前使用对应 `community` 软件源中的 `shadow` 包管理账号。BusyBox
  `deluser` 会同时删除同名组，不能直接替换；未来若支持仅启用 `main` 的主机，必须先
  保证卸载仍只删除项目创建的账号和组。
...
- Ubuntu 24.04/systemd 与 Alpine 3.22/OpenRC 的测试快照仅作为旧版安装器的历史
  ...
  管理；当前 Alpine 主机声明只以 Native 主机矩阵和测试指南中的完整系统验证规则为准。
...
- 使用经过测试的 Go 生命周期引擎及其持久 generation journal，取代各发行版专用的
  Shell 状态修改逻辑。
- 建立以证据为基础的 Native 主机矩阵：Rocky Linux 9 是主要目标，同时明确 systemd
  兼容配置、Alpine/OpenRC 服务路径，以及尚未完成验证的发行版层级。
