<!-- translation: locale=zh-CN; source=docs/development/roadmap.md; source-sha256=a0d6075c7a4e22a5548f79f258d279a4edbc76c8ab4eca5c92c45e7336f82542 -->
# Remnanode Lite 路线图
...
- `/node` 路由严格遵循官方 Node 3.2.2 的 HTTP 方法、请求、响应和错误语义。
...
| M11 官方 Node 3.2.2 对齐 | 实现已完成；等待候选验收 |
...
Zod 4 契约边界。`3.2.2` 源码线已经对齐当前官方契约，包括可选启动 metadata、
integrations 与插件 schema `0.6.3`，发布前仍需完成不可变候选验收。
运行观测不写入源码仓库，Release notes 由 GitHub 自动生成。

## 当前重点

- **当前**：使用兼容 Panel 验证不可变的 `3.2.2` 候选，确认内置 rw-core 启动和真实
  代理流量。对可选启动字段与 Torrent Blocker 报告 webhook 保持聚焦回归覆盖；stock
  构建不增加 Integration 模块。服务器细节与运行观测不写入仓库。
- **发布范围**：未来两个发布周期不新增发布通道、产物类型、发布状态或证明机制；可靠性、
  安全修复和删除重复检查仍在范围内。
- **下一步**：完成 `3.2.2` 发布决策后，继续推进明确列出的 Native 主机验证候选。
