<!-- translation: locale=zh-CN; source=docs/development/roadmap.md; source-sha256=c22c5d69831a71491e72824ac793f96dc1c968bb30ed84f6b585aad26a77a6e3 -->
# Remnanode Lite 路线图
...
- `/node` 路由严格遵循官方 Node 3.3.2 的 HTTP 方法、请求、响应和错误语义。
...
| M12 官方 Node 3.3.2 对齐 | 实现已完成；等待候选验收 |
...
`0.6.3`。`3.3.2` 源码线以派生 SNI、Secret 完整性、GeoCheck `0.3.0`、Torrent
规则位置与插件 schema `0.7.3` 对齐当前官方契约，发布前仍需完成不可变候选验收。
运行观测不写入源码仓库，Release notes 由 GitHub 自动生成。

## 当前重点

- **当前**：使用兼容 Panel 验证不可变的 `3.3.2` 候选，确认派生 SNI、GeoCheck、
  内置 rw-core 启动和真实代理流量。继续对 Secret 完整性与 Torrent 规则位置做聚焦
  回归。服务器细节与运行观测不写入仓库。
- **发布范围**：未来两个发布周期不新增发布通道、产物类型、发布状态或证明机制；可靠性、
  安全修复和删除重复检查仍在范围内。
- **下一步**：完成 `3.3.2` 发布决策后，继续推进明确列出的 Native 主机验证候选。
