<!-- translation: locale=zh-CN; source=docs/development/contract-2.8.0.md; source-sha256=d6abb7ccfe9b9f4f4359db63c5113244a2dcfa08802dcd5edaf5c2e5a298bacb -->
...
Release workflow 只从当前 `main` HEAD 运行。它会解析该提交的 `sha-*` 候选，校验两个可运行 Linux manifest、各平台 SBOM 和 attestation，验证已构建的 Native bundle，创建并校验 draft Release，公开并确认 immutable 后，再在不重建镜像的情况下把同一 digest 晋升为 `2.8.0` 与 `latest`。Release notes 由 GitHub 自动生成。
