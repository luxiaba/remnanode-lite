<!-- translation: locale=zh-CN; source=docs/development/resource-budget.md; source-sha256=4d1291d6e05837b87ab56eda37fc16e41e6c8df5029cb9c6fc5ac5c5d1f200e0 -->
...
本文包含一组带日期的工程测量和当前资源策略。测量结果只对列出的 commit 时代、工具链、架构和测试资产成立，用于提供工程背景，不会自动成为后续发布候选的运行期证据。

## 验收边界

对于 `v2.8.0`，唯一阻塞的资源/运行期 profile 是冻结候选的 `docker-production-smoke-v1`：在 `amd64` 使用固定到 digest 的生产 Compose 部署接入真实 Panel 并通过真实代理流量，记录实际内存与 PID 使用量，确认容器持续运行且健康、OOM kill 与 restart 都为零。[验收协议](release-acceptance.md#docker-生产-smoke)定义完整要求，包括在精确宿主、容器限制、health 和 readiness 阈值下运行至少 600 秒。
...
生产 Compose 使用不同但更贴近实际运行的 tmpfs 布局：`/run`、`/tmp` 和 rw-core 日志合计 `48 MiB`，且日志不写入持久卷。历史门禁的单个 64 MiB `/tmp` 是测试夹具，不应描述成 Compose 已经在该门禁中逐项复现。阻塞发布的 `amd64` smoke 使用生产 Compose 布局，但不要求重复历史 50,000 用户负载。

下列 2026-07-15 M6 数值与 2026-07-19 M7 init 快照早于当前 M8 候选，只作为工程基线保留。它们不是当前候选的运行期证据，重复这些测量也不是发布 `2.8.0` 的前置条件。
...
OpenRC 另外由 supervisor 写入 `openrc.log` 与 `openrc.err.log`。它们每 10 秒巡检并 copy-truncate，成功巡检后 `.1` 以 `4 MiB` 为阈值，但 current 在轮询窗口内可超过阈值，不能宣称数学硬上限。因此 OpenRC 四组 current + `.1` 的阈值预算为 `32 MiB`，四个固定临时文件全部残留时约为 `48 MiB`，再加两个 OpenRC current 在轮询窗口内超过阈值的增量。systemd journal 配置为每 30 秒最多接收 200 条服务日志，但字节和长期磁盘仍服从宿主机 journald 配额。后续扩展验证应在 `2 GB` 整机磁盘下记录日志故障风暴和长期增长；该验证已延后且不阻塞 `2.8.0`，上述阈值也不能替代其结果。
