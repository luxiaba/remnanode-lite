<!-- translation: locale=zh-CN; source=docs/deployment-native.md; source-sha256=2949093a897171d5e768035cd15b73546aab21454af9570ab3ac752d263e9cb2 -->
...
原生部署直接由宿主机的 systemd 或 OpenRC 运行 `remnanode-lite`，适合没有 Docker、或希望省掉 Docker daemon 开销的小型服务器。Docker Compose 仍是大多数节点的默认方式；Native bundle 是正式发行物，不是临时的源码安装脚本。

每个 bundle 都包含 Node、`rnlctl`、rw-core、GeoIP、GeoSite、ASN 数据、服务定义、许可证与 SPDX SBOM，并用 manifest 记录每个文件的摘要。安装器会先校验归档摘要，再校验 bundle 内容，之后才修改主机。

首个完整 Native 生命周期版本计划为 `2.8.0-rnl.1`，目前尚未发布；它实现官方 Node `2.8.0` 契约。Native 安装和升级只接受完整、精确的 `2.8.0-rnl.1` 或 `2.8.0`；`latest`、`preview`、`edge` 和 `sha-*` 等移动名称不能用于 Native。
...
Release bundle 提供 Linux `amd64` 和 `arm64`。服务默认限制为 `448 MiB RAM`、不额外使用 swap、`1 CPU`、`256 tasks`，为 `512 MiB / 1 vCPU / 2 GB` 主机保留余量。OpenRC 还要求 `supervise-daemon`、`checkpath`、`rc-update`、cgroup v2、可写的 memory/CPU/PID controller 和 `cgroup.kill`；缺少这些条件时服务会拒绝启动。
...
```bash
VERSION=2.8.0-rnl.1  # 该预览版发布后再执行
...
`rnlctl` 是独立的 root-owned 普通文件，不是指向当前 generation 的软链接。即使 generation 链接损坏，修复工具仍可运行。服务使用不可登录的 `remnanode` 用户和组；`uninstall --purge` 只会删除安装器创建且身份未被改变的账号对象。
...
`/etc/remnanode-lite/node.env` 是 root 管理的数据文件，不是 shell 脚本。Secret 应放在 `/etc/remnanode-lite/secret.key`，权限为 `root:remnanode 0640`；轮换时写入临时 root-only 文件，再原子替换并重启：
...
- `/etc/remnanode-lite` 目录保持 `root:remnanode 0750`，配置和 Secret 为 `0640`。
