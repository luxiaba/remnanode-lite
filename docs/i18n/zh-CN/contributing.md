...
- `main` 是受保护的发布分支。经过测试的 `dev` 提升合入后，会产生可以正式发布的候选提交。
...
普通 Pull Request 的目标分支是 `dev`。维护者应先在 `dev` 准备好版本、变更日志、文档和测试，再创建 `dev -> main` Pull Request。合入后的 `main` 提交就是发布候选，不再追加仅用于发布资料的最终化提交。不要直接在 `main` 开发，也不要在功能 PR 中创建或移动正式 tag。
...
5. 添加请求、响应、错误和副作用测试。
6. 兼容性风险需要时，使用 `contract-probe` 与真实官方节点比较。
...
生产目标仍是整机 `512 MiB RAM / 1 vCPU / 2 GB disk`。仓库维护的容器通过
`448 MiB` 内存、无额外容器 swap、`1 CPU` 和 `256 PIDs` 的限制为宿主机留出余量。
任何新增资源都必须有界：
...
重跑；带日期的工程基线只说明当时记录的条件。

### 安全与 Secret

- 不记录或提交 `SECRET_KEY`、JWT、客户端证书、私钥和完整认证 header。
- 不把真实节点 IP、hostname、宿主清单、容器标识、日志或原始 Panel 响应写入测试 fixture、文档或仓库。
...
- Bash 脚本使用 `set -euo pipefail`，Alpine/OpenRC 文件保持 POSIX `sh` 兼容。
...
面向使用者的变化更新 `CHANGELOG.md`。不要把未发布版本写成已发布事实，也不要
在多份文档复制容易漂移的“当前状态”；优先链接版本策略、契约或 Release notes 的
单一事实源。
