<!-- translation: locale=zh-CN; source=CONTRIBUTING.md; source-sha256=7f05ec8445307a5876b795f47e7d131094277501c3e574040a6b558f6c7dc3d5 -->
...
- [开发指南](development/README.md)：环境、代码地图和常见修改路径。
- [架构说明](architecture.md)：运行链路、依赖方向、并发与生命周期边界。
- [测试指南](development/testing.md)：测试层级和按改动选择测试。
- [版本策略](versioning.md)：项目版本与官方契约版本的独立语义。
- [文档总览](README.md)：部署、运维、契约和发布资料入口。

发现安全漏洞或 Secret 泄漏风险时，不要先创建公开 Issue；按
[安全策略](security.md)的私密渠道报告，不要公开利用细节。
...
普通 Pull Request 的目标分支是 `dev`。维护者准备发布时先创建 `dev -> main` Pull Request；冻结候选验收完成后，还会按[发布流程](release.md#7-受保护-main-下提交发布资料)创建只含 README、CHANGELOG、roadmap、evidence 和 Release note 的 `release/v*-docs -> main` 最终化 PR。除这两个受控入口外，不要直接在 `main` 开发，也不要在功能 PR 中创建或移动正式 tag。
...
`internal/xrayrpc/wire/wire.pb.go` 是生成文件，不得手工编辑。wire schema 变化必须通过
...
面向使用者的变化更新 `CHANGELOG.md`。不要把未完成验收写成已发布事实，也不要
...
测试范围按风险决定，完整矩阵见[测试指南](development/testing.md#按改动选择测试)。
最低要求：
...
发布门禁要求冻结候选、干净工作树、固定官方源码和当前版本 profile 要求的验收资料。
不要修改检查以绕过缺失的必需 evidence，也不要将 `edge`、`sha-*` 或
`candidate-sha-*` 镜像描述为正式版。

`v2.8.0` 的 M8 门禁要求冻结镜像 digest 在正式发布前通过生产 `amd64` 主机上的
`docker-production-smoke-v1`。阻塞性记录覆盖生产 Compose 模板、预期版本、
真实 Panel 连接、真实代理流量、cgroup 内存/PID 观测、容器持续运行且健康、无 OOM
kill 与 restart 都为零。
`arm64` 运行、`native-systemd-install`、`native-openrc-install`、当前候选
50,000 用户负载、24 小时 soak、
故障/回滚注入均明确延期且不阻塞发布，并须如实披露。

由 operator 签注的运行观测是可追责的审计声明。校验可以把记录绑定到候选 commit 与
digest，并检查 schema 和内部一致性，但不能让底层观测变得不可伪造，也无法独立证明
运行确实发生。不得编造或夸大此类 evidence。
