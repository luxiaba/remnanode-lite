...
本文面向第一次接触 Remnanode Lite 的维护者，目标是让你在不启动真实节点、
不准备 Panel 和 `SECRET_KEY` 的前提下，完成环境准备、代码定位、修改与本地验证。
...
5. 按[测试指南](testing.md)选择最低验证；准备提交时再阅读[贡献指南](../../CONTRIBUTING.md)。

以下资料按改动需要深入：修改 Panel 可见行为时读[当前契约基线](contract-2.8.0.md)，修改版本或镜像时读[版本策略](../versioning.md)，准备正式版本时读[发布手册](../release.md)。[文档总览](../README.md)保存完整角色导航。
...
| `internal/xtls` | 与 rw-core 通信的最小 protobuf/gRPC 客户端 |
...
| 路径 | 职责 |
| --- | --- |
| `.github/workflows/` | CI、容器候选、正式 Release、安全扫描和官方版本监测 |
| `scripts/check*.sh` | Go、仓库、供应链和完整门禁的稳定入口 |
| `scripts/install*.sh`、`upgrade.sh`、`uninstall.sh` | 原生安装、资产事务、升级回滚和卸载 |
| `deploy/` | systemd/OpenRC service 模板与原生 `node.env` 示例 |
| `compose.yaml`、`compose.build.yaml` | GHCR 运行配置与本地源码构建覆盖层 |
| `docs/examples/` | 可直接部署、同时作为 Release 资产输入的完整样本 |
| `docs/development/acceptance/` | 版本化、脱敏并由机器验证的正式验收证据；候选阶段才创建 |
| `docs/releases/` | 与 Git tag 一一对应的 GitHub Release note；候选阶段才创建 |
| `Dockerfile` | 双架构 Node、固定 rw-core/geo/ASN 资产和最小 runtime 镜像 |
| `cmd/docs-check` | Markdown 结构、相对链接、锚点和入口可达性检查 |
...
6. 检查 diff、文档与 CHANGELOG，再提交逻辑完整的 checkpoint。
...
不要每改一行就运行发布门禁。测试成本应与改动风险匹配；完整仓库检查适合一个
逻辑批次完成后或提交 Pull Request 前运行，真实 Panel、资源和长期 soak 属于候选
验收层。
...
| 用户热更新 | `internal/nodehandler`、`internal/xray/handler.go`、`internal/xtls` | gRPC 结果与 hash commit 必须保持事务语义 |
| 统计语义 | `internal/stats`、`internal/xtls/stats.go`、HTTP route | 特别检查 reset、缺失值和官方响应 schema |
...
外部 API 不是自由设计面。不要根据“更合理”的直觉改变状态码、JSON shape、缺失值、
...
`internal/xtls/xrpc/wire.pb.go` 是生成文件，不要手工编辑。仓库使用 `protoc 35.1` 和
`protoc-gen-go v1.36.11`，由固定入口重新生成：

```bash
scripts/generate-protobuf.sh
go test -count=1 ./internal/xtls
