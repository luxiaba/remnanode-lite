...
发行版本固定使用 `<官方 Node 版本>-rnl.<修订号>`。官方发布新版本后，定时 workflow 只创建同步 Issue；完成源码固定、契约差分、实现调整与真实验收后，才发布对应的 `rnl.1`。同一官方兼容基线下的本项目修复递增 `rnl` 修订号，不移动既有 tag。
...
- 正式版本使用不可移动的 `v<官方版本>-rnl.<修订号>` 注释 tag；已发布精确版本不得覆盖。
