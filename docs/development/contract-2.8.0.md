...
路由方法取自四个官方 controller；请求和响应取自 `libs/contract/commands` 下的 Zod schema；应用错误取自 `libs/contract/constants/errors` 和 `HttpExceptionFilter`。CI 会验证官方 Git 提交、包名/版本，以及所有登记证据路径存在且文件非空。它不会比较这些源码文件的内容摘要，也不会证明人工蒸馏出的 Go schema 与 TypeScript 自动等价；升级契约时仍必须评审实际 diff。
