...
For the `3.4.1` release, use the Compose template shipped with that Release:
it selects the `3.4.1` image, includes the release-locked GeoCheck runtime
asset, and explicitly maps the new SNI and nftables options. Keep the existing
`.env`, pull the exact image or digest, and recreate the container before
checking Panel connectivity and real traffic.

Derived-SNI verification is now optional and off by default. TLS 1.3, mTLS,
JWT validation, and Secret integrity remain mandatory. Set
`SNI_VERIFICATION=true` only when every connecting Panel sends the derived
server name. Review `NFTABLES_LOGGING` before rollout on a busy host; keep
`NFTABLES_ACCEPT_REPLY_TRAFFIC=false` unless an ingress list is blocking replies
to host-originated connections and conntrack is available.
