...
cmd/remnanode-lite
  -> httpserver -> nodeapi
                ...
                -> xray
  -> unixconfig
  -> xray       -> xtls / executil / system / netadmin / version
  -> config     -> secret
```
...
daemon 入口是 `runNode`，没有 CLI 参数时由 `main` 调用。启动顺序有意让配置和安全错误在监听端口前失败：

1. 解析运行配置。
   - `REMNANODE_ENV` 显式指定路径时优先。
   - 否则优先已有的 `/etc/remnanode/node.env`，最后回退 `.env`。
   - 配置文件值先加载，已知且非空的进程环境变量再覆盖它们。
2. 配置公开 `/node` server 的请求体预算，并应用经过格式校验的 contract/core 版本 override。
...
7. 创建 `plugin.State`，尝试打开 ASN 数据库。
   - ASN 文件缺失或无效时插件 `asList` 降级为空，daemon 继续启动。
8. 创建 `connections.Dropper` 和 `plugin.Service`。
   - Plugin webhook 单 worker 在 Service 构造时启动。
   - nftables 初始化失败是受支持的降级模式，非 nft 路由仍可服务。
9. 将 `plugin.State` 注入 Manager，供 Xray 配置生成阶段读取 Torrent 设置。
10. 创建公网 HTTPS server 和文件系统 Unix HTTP server。公网 server 在此校验 Node 证书/私钥并构建客户端 CA pool。
11. 并行启动公网 HTTPS、内部 Unix HTTP 和日志轮转，等待 `SIGINT`、`SIGTERM` 或任一 server 异常退出。

`internal/system` 的默认网络 monitor 是包级单例。在 Linux `/proc/net/dev` 可用时，它在包初始化期间启动 goroutine，每 3 秒采样一次；组合根负责在退出时停止它。
...
`nodehandler.Service` 将所有 add/remove mutation 通过容量为 1 的可取消 gate 串行。每次 rw-core Handler RPC 成功后，还要使用发起调用时的 Xray generation 提交本地用户哈希；若期间 core 已被替换或停止，提交会失败，旧 RPC 不能污染新实例状态。
...
Manager 每次启动生成随机 abstract socket 名称，并把 tunnel inbound 注入 rw-core 配置。`internal/xtls` 使用 `grpc.ClientConnInterface.Invoke` 和显式 method path，不依赖完整 Xray Go SDK。
...
10. 确认进程仍属于当前 generation，探测 core 版本，原子发布 running、hash state 和 version。
...
| 状态 | 唯一所有者 | 同步方式 | 生命周期 |
| --- | --- | --- | --- |
| rw-core process/state/generation | `xray.Manager` | `mu` + `lifecycleMu` | Node 进程内 |
| pending Xray JSON | `xray.Manager` | `mu` | 仅 start 到 gRPC ready |
| inbound tags/user hash | `xray.Manager` | `mu` + generation fence | rw-core running generation |
...
```text
HTTP Xray lifecycle lease -> Plugin operation gate -> xray.Manager
```

用户 Handler 不长期持有 HTTP lifecycle lease，而依靠 Manager generation fence 拒绝跨 core 生命周期的陈旧状态提交。未来新增绕过 HTTP 的内部 mutation 入口时，必须明确复用相同协调策略，不能直接调用多个组件并自创锁序。
...
5. 同步 `internal/contract` 的路由、schema、合法样例、副作用和官方 source evidence。
6. 增加“非法输入不触发副作用”以及真实 dispatcher response schema 测试。

### 修改 Xray 生命周期

- 保持 `Manager` 是唯一进程 owner。
- 任何异步结果提交前检查 generation 和 state。
...
- 官方源码证据：设置 `REMNANODE_OFFICIAL_SOURCE` 后运行 `go test ./internal/contract`
...
`internal/contract` 是从固定官方源码人工蒸馏出的可执行 Zod 子集，不是 TypeScript schema 的自动生成物。source evidence 测试负责固定官方 commit、版本和证据文件；需要确认两个运行实例的真实可观测语义时，使用 `cmd/contract-probe` 做受控黑盒比较。
...
| 官方兼容目标与响应 schema | `internal/contract` + 固定官方源码 checkout |
