...
5. 发布结果写入[变更日志](CHANGELOG.md)和对应 Release note；不存在 Git tag 与 Release 资产时，不得把开发版本描述成已发布版本。
...
| [许可证](../LICENSE) | AGPL-3.0-only 许可说明 |
...
| [单文件 Compose 示例](examples/compose.single-file.yaml) | 直接内联运行变量、适合大量独立小节点的完整模板 |
...
| [历史审计整改记录](development/audit-remediation.md) | 首轮静态审计的历史范围；不作为当前状态真相源 |
...
| [变更日志](CHANGELOG.md) | 已发布和待发布的用户可见变化 |
