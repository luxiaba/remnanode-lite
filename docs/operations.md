...
将 `38329` 换成实际 `NODE_PORT`。Compose healthcheck 只执行：

```text
test -S /run/remnanode/internal.sock && kill -0 1
```

因此容器 healthy 但 Panel 离线时，应继续检查端口、防火墙、Secret 和 Panel 配置。
...
| 命令 | 用途 | 注意事项 |
| --- | --- | --- |
| `remnanode-lite version` | 显示项目版本与当前进程将上报的契约版本 | 不解析 `node.env`，但会读取进程环境中的 `NODE_CONTRACT_VERSION`；干净环境下可用于二进制 smoke test |
...
不要通过覆盖旧 tag 实现回滚。确认新版本稳定并保留至少一个回滚镜像后，再清理磁盘：
